<template>
    <div class="competition-form">
        ...
        <el-row :gutter="20" class="mt-10">
            <el-col :span="24">
                <el-upload ref="uploader" class="uploader" :action="uploaderAction" :auto-upload="false"
                    :show-file-list="true" :limit="1" :on-change="onUploaderChange" accept="image/jpeg, image/png"
                    :before-upload="beforeUpload" :on-remove="removeUpload" :http-request="httpRequest">
        ...
        <el-row class="mt-20">
            <el-col :span="24" class="center-content">
                <el-button color="#626aef" plain @click="cancel" :disabled="disabledPost" :loading="postLoading">
                    取消
                </el-button>
                <el-button color="#626aef" plain @click="updateCompetition" :disabled="disabledPost"
                    :loading="postLoading">
                    保存
                </el-button>
            </el-col>
...
import { getCompetitionInfoAPI, releaseCompetitionAPI, updateCompetitionAPI } from "../../../api/competitionAPI";
...
const getCompetitionInfo = async () => {
    ...
    console.log("获取竞赛数据...", res);
    if (res.code === 200) {
        competitionName.value = res.data.competitionName;
        officialWebsite.value = res.data.officialWebsite;
        competitionCategory.value = res.data.categoryId;
        registrationTime.value = [res.data.registrationStart, res.data.registrationEnd];
        competitionTime.value = [res.data.competitionStart, res.data.competitionEnd];
        competitionImg.value = res.data.competitionImg;
        nextTick(() => {
            writeEditor.value?.setValue(res.data.description);
        });
    } else {
...
function cancel() {
    router.replace({ path: '/c/' + props.pid })
}
    ...
    if (!competitionName.value.trim()) {
        ...
        setTimeout(() => {
            ...
            updateCompetitionAPI(
                ...
                postResultDialogTitle.value = '更新成功';
