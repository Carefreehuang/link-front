<template>
  <div class="sys-tag">
    <div class=sys-tag-header>
      ...
      <!-- 添加标签对话框 -->
      <el-dialog v-model="showAddTagDialog" title="添加标签" class="tag-dialog" align-center :lock-scroll="false">
    ...
    formRef.validate((isValid) => {
      if (isValid) {
        addTagAPI(tagForm.label, tagForm.name, tagForm.icon, tagForm.option, tagForm.detail).then(response => {
          ...
          } else if (error.message === '标签英文标识已存在') {
    ...
    formRef.validate((isValid) => {
      if (isValid) {
        addTagOptionAPI(tagOptionForm.label, tagOptionForm.name).then(response => {
          ...
          } else if (error.message === '类型显示名称已存在') {
          ...
          } else if (error.message === '类型英文标识已存在') {
